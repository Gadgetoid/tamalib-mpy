...
u32_t clk_timer_timestamp = 0; // in ticks 
u32_t prog_timer_timestamp = 0; // in ticks
...
static timestamp_t wait_for_cycles(timestamp_t since, u8_t cycles) {
	...
	deadline = since + (cycles * cpu_fs_freq)/(TICK_FREQUENCY * speed_ratio);
...
bool_t cpu_init(const u12_t *program, breakpoint_t *breakpoints, u32_t freq)
{
	for (unsigned i = 0; i < INT_SLOT_NUM; i++) {
		interrupts[i].factor_flag_reg = 0;
		interrupts[i].mask_reg = 0;
		interrupts[i].triggered = 0;
	}
	interrupts[0].vector = 0x0C; // Prog timer
	interrupts[1].vector = 0x0A; // Serial interface
	interrupts[2].vector = 0x08; // Input (K10-K13)
	interrupts[3].vector = 0x06; // Input (K00-K03)
	interrupts[4].vector = 0x04; // Stopwatch timer
	interrupts[5].vector = 0x02; // Clock timer
